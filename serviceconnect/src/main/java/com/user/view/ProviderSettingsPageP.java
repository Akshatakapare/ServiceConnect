...
public class ProviderSettingsPageP extends Application {
    ...
    public void start(Stage stage) {
        ...
        VBox securityCard = sectionGroup("Security", new VBox(passwordBtn));
          styleButton(passwordBtn);
             passwordBtn.setOnAction(e -> {
                changepassP changePage = new changepassP();
                 try {
        ...
        VBox deleteCard = sectionGroup("Delete My Account", new VBox(deleteBtn));
          ...
          deleteBtn.setOnAction(e -> {
            deleteAccP deletePage = new deleteAccP();
              try {
    ...
    // Reset radio buttons to default (Online selected)
    online.setSelected(true);
    offline.setSelected(false);

    // Reset toggles to OFF state
    for (Node node : notifCard.getChildren()) {
        if (node instanceof HBox row) {
            Node toggle = row.getChildren().get(row.getChildren().size() - 1);
            if (toggle instanceof StackPane sp) {
                resetToggle(sp);
            }
        }
    }

    for (Node node : serviceCard.getChildren()) {
        if (node instanceof HBox row) {
            Node toggle = row.getChildren().get(row.getChildren().size() - 1);
            if (toggle instanceof StackPane sp) {
                resetToggle(sp);
    ...
    // Reset ComboBoxes
    langBox.getSelectionModel().clearSelection();
    themeBox.getSelectionModel().clearSelection();
        ...
        scrollPane.setStyle("-fx-background: linear-gradient(to bottom, #fff, #edd5edff);");
    ...
    private void resetToggle(StackPane toggle) {
    if (toggle.getChildren().get(0) instanceof Circle knob) {
        toggle.setStyle("-fx-background-color: #5A5A5A; -fx-background-radius: 50;");
        StackPane.setAlignment(knob, Pos.CENTER_LEFT);
        StackPane.setMargin(knob, new Insets(0, 0, 0, 3));
    }
